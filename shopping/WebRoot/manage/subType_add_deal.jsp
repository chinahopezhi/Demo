...
if (rs.next()){
	out.println("<script language='javascript'>alert('Have added,try a new small department！');window.location.href='sub_add.jsp';</script>");
}else{
	int ret=0;
        SubtypeDaoImpl ins_subType=new SubtypeDaoImpl();
        ret=ins_subType.insert(SubType);
	if (ret!=0){
          out.println("<script language='javascript'>alert('Added successfully！');window.location.href='subType.jsp';</script>");
        }else{
          out.println("<script language='javascript'>alert('Added failed！');window.location.href='sub_add.jsp';</script>");
        }
}
%>